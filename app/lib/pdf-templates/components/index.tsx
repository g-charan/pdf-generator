...
import type { Invoice } from "../types"; // Updated path
...
import { CreditReferenceTemplate } from "./credit-reference";
import { CreditApplicationTemplate } from "./credit-application";
import { LetterOfCreditTemplate } from "./letter-of-credit";
import { useState, useEffect } from "react";

export { CreditReferenceTemplate, CreditApplicationTemplate, LetterOfCreditTemplate };
...
export function PdfTemplate(props: Invoice) {
  const {
    invoiceNumber,
    issueDate,
    dueDate,
  ...
  useEffect(() => {
    if (template.includeQr) {
      QRCodeUtil.toDataURL(`https://app.midday.ai/i/${token}`, {
        margin: 0,
        width: 40 * 3,
      }).then(setQrCode);
    } else {
